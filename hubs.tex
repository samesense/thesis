...
complexes in a time and space dependent manner. Intermodular hub
...
\caption[Comparing hubs across human interaction networks]{\small In
  this study we used three human interaction networks, I2D, STRING,
  and HPRD. To find hubs for each network, we took the top 20\% most
  connected nodes. Each network had a different hub set due to
  differing network size and connectivity. Here we show the overlap
  between hub sets for the three
  networks. \label{fig:sysBio:hubOverlap}}
\end{figure}
...
collection of neighbor co-expression correlations. Using the median
instead of the average did not produce qualitatively different results
for virus-host protein interactions (Table
\ref{tbl:sysBio:bindingFisher}). For the siRNA data, the association
between HCV host dependency factors and intramodular hubs became
significant (Table \ref{tbl:sysBio:rnaiFisher}).
...
to annotate proteins of unknown function with the functions of
